package com.google.android.gms.internal;

import android.os.Binder;
import android.os.IBinder;
import android.os.IInterface;
import android.os.Parcel;
import android.os.RemoteException;
import com.google.android.gms.fitness.result.DataReadResult;

// $FF: renamed from: com.google.android.gms.internal.lw
public interface class_140 extends IInterface {
   // $FF: renamed from: a (com.google.android.gms.fitness.result.DataReadResult) void
   void method_765(DataReadResult var1) throws RemoteException;

   public abstract static class class_1105 extends Binder implements class_140 {
      public class_1105() {
         this.attachInterface(this, "com.google.android.gms.fitness.internal.IDataReadCallback");
      }

      // $FF: renamed from: as (android.os.IBinder) com.google.android.gms.internal.lw
      public static class_140 method_2336(IBinder var0) {
         if(var0 == null) {
            return null;
         } else {
            IInterface var1 = var0.queryLocalInterface("com.google.android.gms.fitness.internal.IDataReadCallback");
            return (class_140)(var1 != null && var1 instanceof class_140?(class_140)var1:new class_140.class_1106(var0));
         }
      }

      public IBinder asBinder() {
         return this;
      }

      public boolean onTransact(int var1, Parcel var2, Parcel var3, int var4) throws RemoteException {
         switch(var1) {
         case 1:
            var2.enforceInterface("com.google.android.gms.fitness.internal.IDataReadCallback");
            DataReadResult var5;
            if(var2.readInt() != 0) {
               var5 = (DataReadResult)DataReadResult.CREATOR.createFromParcel(var2);
            } else {
               var5 = null;
            }

            this.a(var5);
            return true;
         case 1598968902:
            var3.writeString("com.google.android.gms.fitness.internal.IDataReadCallback");
            return true;
         default:
            return super.onTransact(var1, var2, var3, var4);
         }
      }
   }

   private static class class_1106 implements class_140 {
      // $FF: renamed from: le android.os.IBinder
      private IBinder field_4280;

      class_1106(IBinder var1) {
         this.field_4280 = var1;
      }

      // $FF: renamed from: a (com.google.android.gms.fitness.result.DataReadResult) void
      public void method_765(DataReadResult param1) throws RemoteException {
         // $FF: Couldn't be decompiled
      }

      public IBinder asBinder() {
         return this.field_4280;
      }
   }
}
